...
async function build(root, buildOptions = {}) {
    ...
    try {
        const { clientResult, serverResult, pageToHashMap } = await bundle_1.bundle(siteConfig, buildOptions);
        const spinner = ora_1.default();
        spinner.start('rendering pages...');
        try {
            const appChunk = clientResult &&
                clientResult.output.find((chunk) => chunk.type === 'chunk' && chunk.isEntry);
            const cssChunk = (siteConfig.mpa ? serverResult : clientResult).output.find((chunk) => chunk.type === 'asset' && chunk.fileName.endsWith('.css'));
