...
result, appChunk, cssChunk, pageToHashMap, hashMapString) {
    ...
    if (config.mpa && result) {
        const matchingChunk = result.output.find((chunk) => chunk.type === 'chunk' &&
            chunk.facadeModuleId === slash_1.slash(path_1.default.join(config.srcDir, page)));
        if (matchingChunk) {
            if (!matchingChunk.code.includes('import')) {
                inlinedScript = `<script type="module">${matchingChunk.code}</script>`;
                fs_extra_1.default.removeSync(path_1.default.resolve(config.outDir, matchingChunk.fileName));
            }
            else {
