...
const buildMPAClient_1 = require("./buildMPAClient");
...
async function bundle(config, options) {
    ...
    const resolveViteConfig = (ssr) => ({
        ...
        logLevel: 'warn',
        plugins: plugin_1.createVitePressPlugin(root, config, ssr, pageToHashMap, clientJSMap),
        // @ts-ignore
        ssr: {
    ...
    try {
        ...
        [clientResult, serverResult] = await Promise.all([
            config.mpa ? null : vite_1.build(resolveViteConfig(false)),
    ...
    if (config.mpa) {
        ...
        for (const chunk of serverResult.output) {
            if (!chunk.fileName.endsWith('.js')) {
                const tempPath = path_1.default.resolve(config.tempDir, chunk.fileName);
                const outPath = path_1.default.resolve(config.outDir, chunk.fileName);
                await fs_extra_1.default.copy(tempPath, outPath);
            }
        }
        // also copy over public dir
        const publicDir = path_1.default.resolve(config.srcDir, 'public');
        if (fs_extra_1.default.existsSync(publicDir)) {
            await fs_extra_1.default.copy(publicDir, config.outDir);
        }
        // build <script client> bundle
        if (Object.keys(clientJSMap).length) {
            clientResult = (await buildMPAClient_1.buildMPAClient(clientJSMap, config));
        }
    }
